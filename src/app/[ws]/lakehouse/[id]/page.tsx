...
import { TYPE_BADGE, PURPOSE_BADGE, REGISTER_BADGE } from "@/features/entries/components/entryMeta";
...
export default function EntryDetailPage({
  ...
  async function confirmDelete() {
    ...
    if (res.ok) {
      router.push(`/${ws}/lakehouse`);
      router.refresh();
    }
  }

  if (loading) return <p className="text-slate-500 dark:text-slate-400">{dict.common.loading}</p>;
  if (!entry) return <p className="text-red-500">{dict.entries.notFound}</p>;

  if (editing) {
    return (
      <div className="max-w-2xl">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold">{dict.entries.editTitle}</h1>
        </div>
        <EntryForm
          initialData={{
            id: entry.id,
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-2 mb-6 text-sm text-slate-500 dark:text-slate-400">
        <Link href={`/${ws}/lakehouse`} className="hover:text-slate-700 dark:hover:text-slate-200">
          {dict.entries.pageTitle}
        </Link>
        <span>/</span>
      ...
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`text-sm px-3 py-1 rounded font-medium ${TYPE_BADGE[entry.type]}`}>
            {dict.entryMeta.type[entry.type] ?? entry.type}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm text-slate-500 dark:text-slate-400">
          <p>{fmt(dict.entries.created, { d: entry.created })}</p>
          <p>{fmt(dict.entries.updated, { d: entry.updated })}</p>
        </div>
      ...
      <ConfirmDialog
        open={showDelete}
        title={dict.entries.deleteConfirm}
        message={dict.common.irreversible}
        confirmLabel={dict.entries.deleteAction}
        danger
