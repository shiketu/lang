...
export default function HomePage() {
  ...
  const routineText = dict.routine as unknown as Record<
    string,
    ...
    <div className="max-w-6xl mx-auto pb-12">
      ...
      <div className="mb-8">
        <span className="inline-flex items-center gap-1 bg-orange-100 text-orange-600 dark:bg-orange-950/40 dark:text-orange-400 text-xs font-bold px-2.5 py-1 rounded-full mb-3">
          ...
          {streak > 0 ? fmt(dict.home.streakActive, { n: streak }) : dict.home.startToday}
        </span>
        <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-1">
          {dict.home.title}
        </h1>
        <p className="text-slate-500 dark:text-slate-400">{dict.home.subtitle}</p>
      </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
              ...
              {dict.home.routineHeading}
            </h2>
            <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
              {fmt(dict.home.completed, { done: doneCount, total: steps.length })}
            </span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">{txt.desc}</p>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {done ? (
                        ...
                        >
                          <Play className="w-4 h-4 fill-current" /> {dict.home.start}
                        </Link>
                      ) : (
                        <Link
                          href={`/${ws}${step.href}`}
